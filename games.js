...
function matchup(team1, team2) {
    return center(`${team1.toUpperCase()}   vs   ${team2.toUpperCase()}`);
}
...
// FORMAT RECEIPT
// ---------------------------------------------------------------------------
function formatReceipt(matches) {
    const DOUBLE = "=".repeat(WIDTH);
    const THIN   = "-".repeat(WIDTH);
    const lines  = [];

    lines.push(DOUBLE);
    lines.push(center("*  FIFA WORLD CUP 2026  *"));
    lines.push(center(prettyDate(today)));
    lines.push(DOUBLE);
    lines.push("");

    if (matches.length === 0) {
        lines.push(center("No matches today."));
        lines.push(center("Rest day. Touch grass."));
        lines.push("");
        lines.push(DOUBLE);
        lines.push("");
        lines.push("");
    ...
    lines.push(center(`>>  ${matches.length} ${label} today  <<`));
    lines.push("");

    matches.forEach((m, i) => {
        const num   = `MATCH ${String(i + 1).padStart(2, "0")}`;
        const tag   = m.group || (m.num ? `#${m.num}` : "");
        const round = (m.round || "").toUpperCase();

        lines.push(THIN);
        lines.push(splitLine(`  ${num}`, `${tag}  `));
        lines.push(THIN);
        lines.push("");
        lines.push(center(round));
        lines.push("");
        lines.push(matchup(m.team1, m.team2));
        lines.push("");
        lines.push(row("KICKOFF",  m.timeString));
        lines.push(row("VENUE",    m.ground));
        lines.push("");
    });
...
const main = async () => {
    ...
    for (let i=0; i < schedule.matches.length; i++){
        ...
        if (today == new Date(match.date).toISOString().slice(0,10)) {
            const UTCString = match.date + 'T' + match.time.slice(0,5) + ':00-0' + match.time.slice(-1) + ':00'
            const matchTime = new Date(UTCString);
            const timeString = matchTime.toLocaleString();
            match.timeString = timeString;
            arr.push(match)
        }
    }

    console.log(`Found ${arr.length} match(es) for ${today}.`);
    const receipt = formatReceipt(arr);
